...
Page p4 = new(
    "In the previous chapter you learned a lot about arrays and their types. Of course, an array is not the only way of storing data. Another popular and even more powerful group of data structures contains various variants of lists. In this chapter you will see such data structures in action, together with illustrations, explanations and descriptions.");

Page p5 = new(
    "So far, you learned a lot about arrays and lists. However, these structures are not the only ones available. Among others, there is also a group of more specialized data structures, which are called limited access data structures.");
...
while (currentPage is not null)
{
    ...
    Console.WriteLine($"\nQuote from (...)");
    Console.Write(currentPage.Previous is not null ? "< PREV [P]" : GetSpaces(14));
    Console.Write(currentPage.Next is not null ? "[N] NEXT >".PadLeft(76) : string.Empty);
    Console.WriteLine();

    var key = Console.ReadKey(true).Key;
    if (key == ConsoleKey.N && currentPage.Next is not null)
    {
        currentPage = currentPage.Next;
        number++;
    }
    else if (key == ConsoleKey.P && currentPage.Previous is not null)
    {
        ...
        Console.WriteLine("Invalid key - Select [P]revious, [N]ext, or [Q]uit.");
}
